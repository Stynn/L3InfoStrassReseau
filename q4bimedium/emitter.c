...
int main(int argc, char* argv[]){
	...
	while (1 == 1){
		...
		exit(0);
			...
			//Select pour l'attente d'un retour serveur
	...
	while (1 == 1){
		...
		if (emission == 1){
			...
			//Ajout des différents caractère lu du fichier
      		int i;
			...
			if (nb_lu == -1){
				...
				//Si il n'y a plus rien à lire on arrete l'émission
				if (nb_lu == 0)
					emission = 0;
			}
		}
		...
		if (reception == 1){
			...
			if ( nbRecv == -1){
				...
				if (nbRecv-1 == -1){
					...
					//Envoi d'un acquittement + ecriture dans le fichier
					if (emission != 1)
						emission = 1;
					if (nbRecv-1 != 0){
						bufAck[0] = buffer[0];
		...
		while (1 == 1 && emission == 1 && routine == 1){
